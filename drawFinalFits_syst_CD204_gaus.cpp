...
int main(void) {
  ...
  for(unsigned i=0; i<volts.size(); i++) {
    ...
    histo->Draw(); //"pe" per avere points+error
    cruijff->Draw("same");
    ...
    c1->SaveAs(Form("plots/CD%d/%s/%dV/%dV_systFWHMvsFIT_%dnbins_gaus.png",CD_number, misura, volts[i], volts[i],nbins));

    
    float systfwhm = abs(reso_fwhm-reso_fwhm_raw)/(1.*reso_fwhm);
    syst_fwhm.push_back(systfwhm);
    float systfwhm_err = sqrt(((reso_fwhm_err_raw*reso_fwhm_err_raw)/(1.*reso_fwhm*reso_fwhm))+(reso_fwhm_raw*reso_fwhm_raw*reso_fwhm_err*reso_fwhm_err)/(1.*reso_fwhm*reso_fwhm*reso_fwhm*reso_fwhm));
