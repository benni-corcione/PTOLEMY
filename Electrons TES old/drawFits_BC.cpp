...
int main() {
  ...
  std::vector<int> colors = {kAzure+7,kGreen-6,kOrange-3,kRed-3};
...
//std::ofstream ofs("mu.txt");
//ofs << "V (V)  mu (V)  mu_err (V) " << std::endl;
  ...
  for( unsigned i=0; i<volts.size(); i++ ) {
    ...
    TH1D* h1_amp = new TH1D( histname.c_str(), "", 67, 0., 1. );
    tree->Project(histname.c_str(), "amp");

    //magheggi sui limiti del fit --> se sto nella condizione prende 0.5, sennò 0.6
    ...
    //per ora non so cosa fa la x
   
    cruijff->SetLineColor(kPink-1);
    cruijff->SetLineWidth(8); //8 single (metà per multipli insieme)
    ...
    h1_amp->GetXaxis()->SetNdivisions(105);
    ...
    c1->SaveAs(Form("~/Documents/laurea\ magistrale/Tesi\ /capitolo\ 5/%d.pdf", volts[i]));
    ...
    gr_mu->SetPointError( i, 0., muerr );
      ...
      //media corretta con shift di temperatura
    ...
    gr_mucorr->SetPointError( i, 0., muerr*corr );
      ...
      //scrittura su file mu.txt
ofs << volts[i] << " " << mu << " " << muerr << std::endl;
...
da riga 174 */ 
  ...
  */
  ...
  //range del grafico finale di risoluzione
...
//correzioni da applicare per il cambio di temperatura del TES
float getCorr( double v, TGraphErrors* gr_temp_vs_V, TGraphErrors* gr_corr ) {
//temperatura
  float temp = -1.;

    //scan sui punti del grafico
  for( unsigned i=0; i<gr_temp_vs_V->GetN(); ++i ) {

      //dichiarazione del nome x, y
    double x, y;
    gr_temp_vs_V->GetPoint( i, x, y ); //prende valore di temperatura ad un dato voltaggio v
  ...
  for( unsigned i=1; i<gr_corr->GetN(); ++i ) {
    ...
    //opero solo se temp compresa tra due punti scelti
    //per applicare la giusta correzione
    if( temp>x0 && temp<x1 ) {
