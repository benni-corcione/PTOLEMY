...
int main() {
  ...
  while( ifs188 >> v1 >> i1 >> r1 >> p1 ) {
    ...
    //gr_power188->SetPointError( iPoint, 0., 0.0025*p1*1E+12/norma1 ); // in pW
    gr_power188->SetPointError( iPoint, 0., 0); // in pW
    iPoint++;
    //errore di 0.5% da ites_ibias che divido per due per coprire tutto con +-
  }
  ...
  while( ifs222 >> v3 >> i3 >> r3 >> p3 ) {
    ...
    //gr_power222->SetPointError( iPoint, 0., 0.007*p3*1E+12/norma3 ); // in pW
    gr_power222->SetPointError( iPoint, 0., 0); // in pW
    iPoint++;
    //errore di 1.5% da ites_ibias che divido per due per coprire tutto con +-
  }
