...
int main(void) {
  ...
  std::vector<int> colors = {92,46,38,46,38};
  std::vector<TH1F*> histos;
  std::vector<TF1*>  crystal_ball;
  ...
  //SEGNARE A MANO IL NOME DEL VOLTAGGIO DESIDERATO
   //drawing of superimposed histogram range
   for( unsigned i=0; i<3; ++i ) {
    int j = index_range[i];
   
    histos[j]->SetMarkerSize(3);
    histos[j]->SetLineWidth(3);
    histos[j]->SetTitle("103 V");
    histos[j]->SetLineColor(kBlack);
    histos[j]->GetXaxis()->SetRangeUser(0.57,0.635);
    histos[j]->GetYaxis()->SetRangeUser(0,300);
    
    gStyle->SetOptStat(0);
    crystal_ball[j]->SetLineColor(colors[j]);
    crystal_ball[j]->SetLineWidth(5);
    histos[j]->Draw("same");
    crystal_ball[j]->Draw("same"); 
  }
   c1->SaveAs(Form("plots/CD%d/%s/103V/superimposed_range_103V.png",CD_number,misura));
   c1->Clear();
   ...
   for( unsigned i=0; i<3; ++i ) {
    ...
    histos[j]->SetMarkerSize(3);
    histos[j]->SetLineWidth(3);
    histos[j]->SetFillStyle(3004);
    histos[j]->SetFillColor(colors[j]);
    histos[j]->SetTitle("103 V");
    histos[j]->SetLineColor(colors[j]);
    histos[j]->GetXaxis()->SetRangeUser(0.57,0.635);
    histos[j]->GetYaxis()->SetRangeUser(0,330);
