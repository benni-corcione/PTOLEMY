...
int main(int argc, char* argv[]){
  ...
  for(int c=0; c<cnt_radius.size(); c++){
	...
	*/
	...
	std::string save_fold(Form("plots/%s/%s/disaligned",folder,subfolder));
	std::string save_subfold(Form("plots/%s/%s/disaligned/%dum",folder,subfolder,cnt_radius[c]));
	system( Form("mkdir -p %s", save_fold.c_str()) );
	system( Form("mkdir -p %s", save_subfold.c_str()) );
      
	c1->SaveAs(Form("plots/%s/%s/disaligned/%dum/%s_energy.png",folder,subfolder,cnt_radius[c],name.c_str()));
	c1->Clear();
