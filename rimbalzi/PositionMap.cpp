	...
	for(int r=0; r<cnt_radius.size(); r++){
	  for(int v=0; v<volts.size(); v++){
	    for(int e=0; e<energy.size(); e++){
	      ...
	      //rimb_d600um_cnt15um_sub5000um_V30_E10-1
		...
		map->Draw("axis");
		ex1->Draw();
		map->SetTitle(Form("% d arrivals - cnt: %d um, d: %d um, sub=%d, V=%d V, E=%d eV",discesa, cnt_radius[r], distance[d], substrate[b], volts[v], energy[e]));
		...
		map2->Draw("axis");
		ex3->Draw();
		map2->SetTitle(Form("Hits start - cnt: %d um, d: %d um, sub=%d, V=%d V, E=%d eV",cnt_radius[r], distance[d], substrate[b], volts[v], energy[e]));
