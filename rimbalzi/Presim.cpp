	...
	for(int r=0; r<cnt_radius.size(); r++){
	  //for(int dg=0; dg<dist_gen.size(); dg++){
		...
		yBins[i] -= pixel_side/2;
	      ...
	      int rangepresim[6] = {400,450,500,600,700,900};
	      ...
	      TH1F* energy = new TH1F("energy","",202,-1,100);	
		...
		std::cout << discesa << " " << hits << std::endl;	  
	      ...
	      map->Draw("axis");
	      ex1->Draw();
	      map->SetTitle(Form("%.2f%% arrivals - cnt: %d um, V=%d V",discesa*100./nentries, cnt_radius[r], volts[v]));
	      map->SetTitle(Form("cnt: %d um", cnt_radius[r]));
	      ...
	      DrawPad(map_zoom,rangepresim[r], tes_side, cnt_radius[r]);
		
	      //disegno totale a sx
	      pad3->cd();
	      map2->Draw("axis");
	      ex3->Draw();
	      map2->SetTitle(Form("Hits start - cnt: %d um, V=%d V",cnt_radius[r], volts[v]));
	      ...
	      std::string save_fold(Form("plots/%s",folder));
	      std::string save_subfold(Form("plots/%s/%s",folder,subfolder));
	      std::string save_subsubfold(Form("plots/%s/%s/maps",folder,subfolder));
	      system( Form("mkdir -p %s", save_fold.c_str()) );
	      system( Form("mkdir -p %s", save_subfold.c_str()));
	       system( Form("mkdir -p %s", save_subsubfold.c_str()));
	      ...
	      fitfunction.SetParLimits(0,0.001,50);
	      fitfunction.SetParLimits(1,0.02,10);
	      fitfunction.SetParLimits(2,0.001,200);
	      fitfunction.SetParLimits(4, 10,10000);
	      fitfunction.SetParLimits(3, 0.1,10);
	      
	      //energy->GetXaxis()->SetRange(,5);
	      energy->Fit("fitfunction","RmX","",0,5);
	      energy->Draw();
...
int GetArrivalRangeZoom(int cnt_radius){
  ...
  if(cnt_radius==200){range = 150;}
  if(cnt_radius==300){range = 150;}
  if(cnt_radius==500){range = 150;}
  return range;
}
...
int GetArrivalRange(int cnt_radius){
  int range;
  /*
  if(cnt_radius==15 ){range = 3000;}
  if(cnt_radius==40 ){range = 7000;}
  if(cnt_radius==100){range = 7000;}
  if(cnt_radius==200){range = 7000;}
  if(cnt_radius==300){range = 7000;}
  if(cnt_radius==500){range = 7000;}
  */
  return range=3000;
}
