...
int main(int argc, char* argv[]){
  ...
  TH1F* strikes_energy = new TH1F("strikes_energy","",100,0,102);
  ...
  gStyle->SetOptStat(0);
  strikes_energy->SetFillStyle(3354);
  strikes_energy->SetFillColor(53);
  strikes_energy->GetYaxis()->SetLabelSize(0);
  strikes_energy->SetTitle("Spectrum of all strikes on uniform spatial event generation");
  strikes_energy->GetXaxis()->SetTitle("Energy (eV)");
  strikes_energy->GetYaxis()->SetTitle("Strikes (a.u.)");
  strikes_energy->SetLineWidth(2);
  strikes_energy->SetLineColor(53);
  strikes_energy->Draw();
  c2->SaveAs(Form("plots/%s/%s/energy_strikes.png",folder,subfolder));
  c2->Clear();
