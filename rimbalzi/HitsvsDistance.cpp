#include "TTree.h"
#include "TFile.h"
#include "TGraphErrors.h"
#include "TMultiGraph.h"
#include "TGraph.h"
#include "TCanvas.h"
#include "TH1F.h"
#include "TStyle.h"
#include "TLegend.h"

#include <fstream>
#include <string>
#include <stdlib.h>
#include <sstream>
#include <filesystem>
#include <algorithm>
#include <iostream>
#include <cmath>

#include "sim_details.h"

void bubbleSort(std::vector<double>& v_e, std::vector<int>& cnts, std::vector<double>& hits, std::vector<int>& sub_vec);
std::vector<int> GetColors(void);
std::vector<float> GetNorm(float binsize, float hist_min, float hist_max);
int main(int argc, char* argv[]){
  
  //parametri da input
  if( argc!= 4 ) {
    std::cout << "USAGE: ./HitsvsDistance [folder] [subfolder] [prefix]" << std::endl;
    std::cout << "EXAMPLE: ./HitsvsDistance rimbalzi rimbalzi_1 rimb" << std::endl;
    exit(1);
  }

  char* folder    = argv[1];
  char* subfolder = argv[2];
  char* prefix    = argv[3];   

  TCanvas* c1 = new TCanvas("c","",1000,1000);
  c1->cd();

  //quantità variabili da ciclare
  std::vector<int> volts      = GetVoltages (folder,subfolder);
  std::vector<int> sim        = GetSimNumber(folder,subfolder);
  std::vector<int> energy     = GetEnergy   (folder,subfolder);
  std::vector<int> cnt_radius = GetCNTs     (folder,subfolder);
  std::vector<int> distance   = Getdistance (folder,subfolder);
  std::vector<int> substrate  = GetSubstrate(folder,subfolder);

  //quantità per futuri grafici comparativi
  std::vector<int> cnt_vec;
  std::vector<double> canestro;
  std::vector<double> VsuE_vec;
  std::vector<int> sub_vec;
  
  //step di 5um
  int tes_side = 60;       //um
  int unit     = 5;        //unit in simulation: 1 is 5 um
  int hist_min = 0;      //questioni grafiche
  int hist_max = 24*unit;  //120 um
  int x_tes    = 110*unit; //um
  int nbins    = 24;

  float binsize = (hist_max-hist_min)/nbins*1.0;
  
  for(int d=0; d<distance.size(); d++){
    for(int b=0; b<substrate.size(); b++){
      for(int s=0; s<sim.size(); s++){
	for(int r=0; r<cnt_radius.size(); r++){
	  for(int v=0; v<volts.size(); v++){
	    for(int e=0; e<energy.size(); e++){
	      if(volts[v]>=energy[e]){
		
		//rimb_d600um_cnt15um_V30_E10-1
		std::string name = std::string(prefix)+ "_d"
		  + std::to_string(distance[d]) + "um_cnt"
		  + std::to_string(cnt_radius[r]) + "um_sub"
		  + std::to_string(substrate[b]) + "um_V"
		  + std::to_string(volts[v]) + "_E"
		  + std::to_string(energy[e]) + "-"
		  + std::to_string(sim[s]);
		
		//apertura file con il tree
		TFile run(Form("root/%s/%s/%s.root",folder,subfolder,name.c_str()));
		if (!run.IsOpen()) {;}
		else{
		  
		  TTree *tree = (TTree*)run.Get(Form("tree"));
		  Long64_t nentries = tree->GetEntries();
		  float x_pos, y_pos, z_pos;
		  float y_in , z_in ;
		  
		  tree->SetBranchAddress("x_f", &x_pos);
		  tree->SetBranchAddress("y_f", &y_pos);
		  tree->SetBranchAddress("z_f", &z_pos);
		  tree->SetBranchAddress("y_i", &y_in);
		  tree->SetBranchAddress("z_i", &z_in);
		  
		  //histo casistica per casistica della distanza per colpire
		  TH1F* hits = new TH1F("hits","",nbins,hist_min,hist_max);
		  
		  for(int iEntry=0; iEntry<nentries; iEntry++){
		    tree->GetEntry(iEntry);
		    x_pos *= unit;
		    y_pos *= unit;
		    z_pos *= unit;
		    
		    //canestro
		    if(x_pos == x_tes &&
		       y_pos<tes_side*0.5 && y_pos>-tes_side*0.5 &&
		       z_pos<tes_side*0.5 && z_pos>-tes_side*0.5){
		      
		      y_in*=unit;
		      z_in*=unit;
		      
		      //calcolo distanza
		      float distance = sqrt(y_in*y_in+z_in*z_in);
		      hits->Fill(distance);
		    } //canestro
		  }//for sul tree
		  
		  //da histo a grafico rinormalizzato, vettori per riempire dinamicamente
		  std::vector<float> geom_factor = GetNorm(binsize,hist_min,hist_max);
		  std::vector<float> norm_counts_vec;
		  std::vector<float> errory_vec;
		  std::vector<float> halfbin_vec;
		  std::vector<int> index;
		  
		  float tot = 0;
		  int counts = 0;
		  
		  for(int i=0; i<nbins; i++){
		    counts = hits->GetBinContent(i+1);
		    if(counts!=0){index.push_back(i);}
		    norm_counts_vec.push_back(1.0*counts/geom_factor[i]);
		    errory_vec.push_back(1.0*sqrt(counts)/geom_factor[i]);  
		    tot += norm_counts_vec[i];
		    halfbin_vec.push_back(hits->GetXaxis()->GetBinCenter(i+1));
		  }
		  
		  for(int i=0; i<nbins; i++){
		    norm_counts_vec[i] = norm_counts_vec[i]/(1.*tot);
		    errory_vec[i] = errory_vec[i]/(1.*tot);
		  }
		  
		  
		  int size = index.size();
		  
		  float norm_counts[size+1];
		  float errory[size+1];
		  float errorx[size+1];
		  float halfbin[size+1];
		  int a = -1; 
		  
		  for(int j=0; j<nbins; j++){
		    if(norm_counts_vec[j]!=0){
		      a++;
		      norm_counts[a]=norm_counts_vec[j];
		      errory[a]=errory_vec[j];
		      halfbin[a]=halfbin_vec[j];
		      errorx[a]=0;
		      
		    }
		  }
		  
		  norm_counts[size]=0;
		  halfbin[size]=1000;
		  errorx[size]=0;
		  errory[size]=0;
		  
		  TGraphErrors g(size+1, halfbin, norm_counts, errorx, errory);
		  c1->SetLeftMargin(0.13);
		  gStyle->SetOptStat(0);
		  g.SetMarkerColor(kAzure+7);
		  g.SetLineColor(kAzure+7);
		  g.SetMarkerStyle(8);
		  g.SetMarkerSize(2);
		  g.SetLineWidth(3);
		  g.GetYaxis()->SetTitle("Strike relative frequency");
		  g.GetXaxis()->SetTitle("Distance from TES [um]");
		  g.SetTitle("Strikes vs distance");
		  
		  g.GetXaxis()->SetLimits(0,120);
		  g.Draw("ape");
		  
		  std::string outdir( Form("plots/%s/%s",folder,subfolder));
		  system( Form("mkdir -p %s", outdir.c_str()) );
		  //-p crea anche le parent se prima non esistono
	      
		//c1->SaveAs(Form("%s/histo_hitstrike/%s_hitsdist.png",outdir.c_str(),name.c_str()));
		  c1->Clear();
		  
		  //salvo numero di canestri, V/E e cnt_radius
		  canestro.push_back(hits->GetEntries());
		  cnt_vec.push_back(cnt_radius[r]);
		  sub_vec.push_back(substrate[b]);
		  VsuE_vec.push_back(1.*energy[e]/volts[v]);

		 
		  //delete(hits);
		}
	      } //else sull'esistenza del file
	    }//for energia
	  }//for voltaggio					
	}//for cnts_radius
      }//for sim
    }//for substrato
  }//for distanze
  
  //lavoro su grafici comparativi a partire delle quantità che ho salvato, che rendo array
 
  //PREPARAZIONE ARRAY PER GRAFICI
  bubbleSort(VsuE_vec, cnt_vec, canestro, sub_vec);
  int Npoints = canestro.size();
  //std::cout << "VsuE_vec      cnt_vec     sub_vec  canestri " << std::endl;
 
 
  //for(int i=0; i<Npoints; i++){
  //std::cout << VsuE_vec[i] << " " << cnt_vec[i] << " " << sub_vec[i] << "  " << canestro[i] << std::endl;
  //}
  int colour[6]= {kRed-7,94,91,kGreen-6,kAzure+7,kViolet-9};
  TLegend* legend = new TLegend(0.6,0.12,0.9,0.3);
  //TLegend* legend = new TLegend(0.6,0.70,0.9,0.88);
  legend->SetBorderSize(0);
  
  //1: hits vs V/E con cnt_radius e sub fissato (canestri percentuali)
  for(int b=0; b<substrate.size(); b++){
    TMultiGraph *mg = new TMultiGraph(); //più plot insieme
    for(int r=0; r<cnt_radius.size(); r++){
      
      //ciclo sui singoli punti acquisiti a parità di cnt_radius
      std::vector<double> x, y; 
      int points=0;
      
      //riempio vettori
      for(int i=0; i<Npoints; i++){ 
	if(cnt_radius[r]==cnt_vec[i] && substrate[b]==sub_vec[i]){
	  points++;
	  x.push_back(VsuE_vec[i]);
	  y.push_back(canestro[i]);
	}//if sul cnt_radius giusto
      }//for sugli Npoints da scorrere

      double density_factor = cnt_radius[r]*cnt_radius[r]/(15.*15.);
      double total_electrons = 50000*density_factor;
      //converto vettori in array per grafico
      double x_[points], y_[points];
      for(int i=0; i<points;i++){
	x_[i]=x[i];
	//y_[i]=y[i]*density_factor;
	y_[i]=y[i]*density_factor*100/total_electrons;
	//y_[i]=y[i];
	//std::cout << "cnt_radius: " << cnt_radius[r] << "  " << x[i] << " " << y_[i] << std::endl;
      }
      //std::cout << std::endl;
      TGraph* hitsVsuE = new TGraph(points,x_,y_);
      hitsVsuE->GetYaxis()->SetRangeUser(1E-2,1E+2);
      hitsVsuE->SetMarkerColor(colour[r]);
      hitsVsuE->SetMarkerStyle(8);
      hitsVsuE->SetMarkerSize(2);
      legend->AddEntry(hitsVsuE,Form("cnt_radius = %d um", cnt_radius[r]),"p");
      mg->Add(hitsVsuE,"p");
      
    }
    
    c1->SetLogy();
    //c1->SetLogx();
    c1->SetLeftMargin(0.11);
    mg->GetYaxis()->SetRangeUser(1E-2,1E+2);
    //mg->GetXaxis()->SetRangeUser(-5E-2,1.05);
    mg->GetXaxis()->SetTitle("E/V");
    mg->GetYaxis()->SetTitle("Strikes");
    mg->GetHistogram()->SetTitle(Form("Strikes vs E/V, substrate = %d um",substrate[b]));
    mg->Draw("a");
    legend->Draw("same");
    c1->SaveAs(Form("plots/%s/%s/canestri/HitsvsVE_sub%d.png",folder,subfolder,substrate[b]));
    c1->Clear();
    legend->Clear(); 
  }


  /*
  //2: hits vs cnt_radius con V/E e substrate fissato
  //inizializzazione di un vettore che ha dentro i numeri di V/E presenti
  std::vector<double> VsuE_list;
  VsuE_list.push_back(VsuE_vec[0]);
  
  for(int i=1; i<Npoints; i++){
    if(VsuE_vec[i-1]!=VsuE_vec[i]){
      VsuE_list.push_back(VsuE_vec[i]);
    }
  }

  
  TLegend* legend2 = new TLegend(0.15,0.12,0.35,0.40);
  legend2->SetBorderSize(0);
  //std::vector<int> colori = GetColors();
  std::vector<int> colori{kRed-7,94,91,kGreen-6,kAzure+7,62,kViolet-9};
  for(int b=0; b<substrate.size(); b++){
    TMultiGraph *mg2 = new TMultiGraph(); //più plot insieme
    for(int r=0; r<VsuE_list.size(); r++){
      
      //ciclo sui singoli punti acquisiti a parità di cnt_radius
      std::vector<double> x, y; 
      int points=0;
      
      //riempio vettori
      for(int i=0; i<Npoints; i++){ 
	if(VsuE_list[r]==VsuE_vec[i] && substrate[b]==sub_vec[i]){
	  points++;
	  x.push_back(cnt_vec[i]);
	  y.push_back(canestro[i]);
	}//if sul VsuE giusto
      }//for sugli Npoints da scorrere
      
      //converto vettori in array per grafico
      double x_[points], y_[points];
      for(int i=0; i<points;i++){
	x_[i]=x[i]; y_[i]=y[i];
      }
      
      TGraph* hitscnts = new TGraph(points,x_,y_);
      hitscnts->GetYaxis()->SetRangeUser(1,1E+5);
      hitscnts->SetMarkerColor(colori[r]);
      hitscnts->SetLineColor(colori[r]);
      hitscnts->SetMarkerStyle(8);
      hitscnts->SetLineWidth(2);
      hitscnts->SetMarkerSize(2);
      legend2->AddEntry(hitscnts,Form("V/E = %.2f", VsuE_list[r]),"pl");
      mg2->Add(hitscnts,"pl");
      
    }
    
    c1->SetLogy();
    c1->SetLeftMargin(0.11);
    //c1->SetRightMargin(0.);
    
    mg2->GetYaxis()->SetRangeUser(1,6E+4);
    mg2->GetXaxis()->SetTitle("cnt radius [um]");
    mg2->GetYaxis()->SetTitle("Canestri");
    mg2->GetHistogram()->SetTitle(Form("Canestri vs cnt_radius, substrate = %d um", substrate[b]));
    mg2->Draw("a");
    legend2->Draw("same");
    c1->SaveAs(Form("plots/%s/%s/canestri/Hitsvscnts_sub%d.png",folder,subfolder,substrate[b]));
    c1->Clear();
    legend2->Clear();
  }

  
  
  TLegend* legend3 = new TLegend(0.7,0.12,0.9,0.26);
  legend3->SetBorderSize(0);

  //3: hits vs substrate con cnt_radius e V/E
   for(int v=0; v<VsuE_list.size(); v++){
    TMultiGraph *mg3 = new TMultiGraph(); //più plot insieme
    for(int r=0; r<cnt_radius.size(); r++){
      
      //ciclo sui singoli punti acquisiti a parità di substrate
      std::vector<double> x, y; 
      int points=0;
      
      //riempio vettori
      for(int i=0; i<Npoints; i++){ 
	if(cnt_radius[r]==cnt_vec[i] && VsuE_list[v]==VsuE_vec[i]){
	  points++;
	  x.push_back(sub_vec[i]);
	  y.push_back(canestro[i]);
	}//if sul cnt_radius giusto
      }//for sugli Npoints da scorrere
      
      //converto vettori in array per grafico
      double x_[points], y_[points];
      for(int i=0; i<points;i++){
	x_[i]=x[i]; y_[i]=y[i];
      }
    
      TGraph* hitssub = new TGraph(points,x_,y_);
      hitssub->GetYaxis()->SetRangeUser(1,1E+5);
      hitssub->SetMarkerColor(colour[r]);
      hitssub->SetMarkerStyle(8);
      hitssub->SetMarkerSize(2);
      legend3->AddEntry(hitssub,Form("cnt_radius = %d um", cnt_radius[r]),"pl");
      mg3->Add(hitssub,"pl");
      
    }
    
    c1->SetLogy();
    c1->SetLeftMargin(0.11);
    mg3->GetYaxis()->SetRangeUser(1,1E+5);
    mg3->GetXaxis()->SetTitle("substrate");
    mg3->GetYaxis()->SetTitle("Canestri");
    mg3->GetHistogram()->SetTitle(Form("Canestri vs substrate, V/E = %.2f",VsuE_list[v]));
    mg3->Draw("a");
    legend3->Draw("same");
    c1->SaveAs(Form("plots/%s/%s/canestri/Hitsvssub_VsuE=%.2f.png",folder,subfolder,VsuE_list[v]));
    c1->Clear();
    legend3->Clear();
   }
  */
  
  delete(c1);
  return(0);
}



void bubbleSort(std::vector<double>& v_e, std::vector<int>& cnts, std::vector<double>& hits, std::vector<int>& sub_vec){
  int n = v_e.size();
  
  // Outer loop that corresponds to the number of
  // elements to be sorted
  for (int i=0; i < n-1; i++) {
     bool swapped = false;
    for (int j=0; j < n-i-1; j++) {
      if (v_e[j] > v_e[j+1]){
	std::swap(v_e[j], v_e[j+1]);
	std::swap(cnts[j], cnts[j+1]);
	std::swap(hits[j], hits[j+1]);
	std::swap(sub_vec[j], sub_vec[j+1]);
	swapped=true;
      }
    }
     if (!swapped) break;
  }
}


std::vector<int> GetColors(void){
  std::vector<int> colors;
  colors.push_back(kRed+3);
  colors.push_back(kRed+1);
  colors.push_back(kRed-7);//
  colors.push_back(kOrange+8);
  colors.push_back(kOrange+1);
  colors.push_back(kOrange);//
  colors.push_back(89); //giallo
  colors.push_back(kSpring-9);
  colors.push_back(79); //verde chiaro
  colors.push_back(kGreen-3);
  colors.push_back(kTeal-7);//
  colors.push_back(kCyan+1);
  colors.push_back(kAzure+10);
  colors.push_back(kAzure+1);
  colors.push_back(kAzure-5);//
  colors.push_back(kBlue-2);
  colors.push_back(kViolet+1);
  colors.push_back(kViolet-9);
  colors.push_back(kMagenta+1);
  colors.push_back(kPink+6);
  return colors;
}
 
//devo avere l'area del settore circolare per rinormalizzare
std::vector<float> GetNorm(float binsize, float hist_min, float hist_max){
  std::vector<float> norm;
  int nsteps = (hist_max-hist_min)/binsize;
  
  for(int i = 0; i<nsteps; i++){
    float radius1 = hist_min+(i+0)*binsize;
    float radius2 = hist_min+(i+1)*binsize;
    norm.push_back(M_PI*(radius2*radius2-radius1*radius1));
  }
  return norm;
}
