...
int main(int argc, char* argv[]){
  ...
  //parametri da input
  if (argc!=5){
    std::cout << "USAGE: ./makeTreePhotons_10k [CD_number] [measure] [power] [trigger]" << std::endl;
  ...
  Controls_10k ctrl; //classe definita in pshape_functions.h
  int ctrl_double;
  int ctrl_trigger;
  int ctrl_width;
  ...
  for(unsigned iEntry=0; iEntry<nentries; iEntry++){
    ...
    baseline        = GetBaseline_10k(pshape);
    baseline_error  = GetBaselineError_10k(pshape, baseline);
    charge          = GetCharge_10k(pshape, sampling_raw, baseline);
    amp             = GetAmp_10k(pshape, baseline);

    ctrl         = Ctrl_pshape_10k(pshape, amp, trigger, baseline);
    ctrl_double  = ctrl.get_ctrl_double_10k();
    ctrl_trigger = ctrl.get_ctrl_trigger_10k();
    ctrl_width   = ctrl.get_ctrl_width_10k();
