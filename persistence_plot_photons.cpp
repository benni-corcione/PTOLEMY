...
int main( int argc, char* argv[] ) {
  ...
  //parametri da input
  if( argc!= 4 ) {
    std::cout << "USAGE: ./persistence_plot [CD_number] [misura] [potenza]" << std::endl;
    exit(1);
  }
  ...
  for(unsigned iEntry=40999; iEntry<41999; iEntry++){
    ...
    h1->GetYaxis()->SetRangeUser(-0.4,0.1);
  ...
  //pshapes
  c1->SaveAs(Form("%s/persistance_pshapes_lumi41.png",outdir.c_str()));
  delete(c1);
