...
int main( int argc, char* argv[] ) {
   ...
   //parametri da input
  if( argc!= 6 ) {
    std::cout << "USAGE: ./checkpshape_10k [CD_number] [misura] [potenza] [event] [lumi] " << std::endl;
    exit(1);
  }
  ...
  std::string fileName = "CD"+ std::to_string(CD_number)
                         + "_" + pw_char + "mW_treeraw.root";
  
  //preparazione lettura del tree
  TFile* file = TFile::Open(Form("data/root/CD%d/%s/%smW/%s",CD_number, meas, pw_char, fileName.c_str()));
  ...
  for(unsigned iEntry=0; iEntry<nentries; iEntry++){
    ...
    if(ev==event && lum==lumi){
      TH1D* h1 = new TH1D("h1", "", 10000,0.,10000*sampling*1.E+6);
      check++;
	...
	h1->SetBinContent( i+1, pshape[i] );
      ...
      h1->GetYaxis()->SetRangeUser(-1.4,0.2);
      //h1->GetYaxis()->SetRangeUser(-0.3,0.05); //dark counts
  ...
  */
  ...
  std::string outdir(Form("plots/CD%d/%s/%smW", CD_number, meas, pw_char));
  system(Form("mkdir -p %s", outdir.c_str()));
  //-p crea anche le parent se prima non esistono
  
  c1->SaveAs(Form("%s/event%dlumi%d_3.png",outdir.c_str(),event,lumi));
