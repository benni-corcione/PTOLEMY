...
adjustable V3_begin = -2000   -- First V2 voltage
adjustable V2_begin = -2000   -- First V3 voltage
adjustable V3_step = 50   -- V3 voltage step size
adjustable V2_step = 50   -- V2 voltage step size
adjustable V3_count = 48  -- V3 voltage number of steps
adjustable V2_count = 48  -- V2 voltage number of steps
...
local function excel_begin()
  ...
  excel_ws.Cells(2, 1).Value2 = "Ek\\V"
end
...
apertura=0
      ...
      -- Record results.
--      print('result: V3='..V3_voltage..',V2='..V2_voltage..',nhits='..nhits..',AngoloI='..angoloInternoMax..'°,esterni='..esterni..',AngoloE='..angoloEsternoMax..'°')
      if excel_enable ~= 0 then
        ...
        excel_ws.Cells(2+V2_count*2, V3_pos+1).Value2 = V3_voltage
        excel_ws.Cells(V2_pos+2+V2_count*2, 1).Value2 = V2_voltage
        excel_ws.Cells(V2_pos+2+V2_count*2, V3_pos+1).Value2 = esterni
		
        excel_ws.Cells(2+V2_count*4, V3_pos+1).Value2 = V3_voltage
        excel_ws.Cells(V2_pos+2+V2_count*4, 1).Value2 = V2_voltage
        excel_ws.Cells(V2_pos+2+V2_count*4, V3_pos+1).Value2 = angoloInternoMax

        excel_ws.Cells(2+V2_count*6, V3_pos+1).Value2 = V3_voltage
        excel_ws.Cells(V2_pos+2+V2_count*6, 1).Value2 = V2_voltage
        excel_ws.Cells(V2_pos+2+V2_count*6, V3_pos+1).Value2 = angoloEsternoMax

        excel_ws.Cells(2+V2_count*8, V3_pos+1).Value2 = V3_voltage
        excel_ws.Cells(V2_pos+2+V2_count*8, 1).Value2 = V2_voltage
        excel_ws.Cells(V2_pos+2+V2_count*8, V3_pos+1).Value2 = aperturaMax
    ...
    simion.printer.filename = 'output/result_' .. V3_voltage .. ", " .. V2_voltage .. '.png'
    ...
    -- Write data to file.
    results_file:write(V3_voltage .. ", " .. V2_voltage .. "\n")
    results_file:flush()  -- immediately output to disk
    if html then
      html:add_image('result_' .. V3_voltage .. ", " .. V2_voltage .. '.png')
      html:add_line('result: V3='..V3_voltage..',V2='..V2_voltage..',nhits='..nhits..',AngoloI='..angoloInternoMax..'°,esterni='..esterni..',AngoloE='..angoloEsternoMax..'°'..',aperturaMax='..aperturaMax..'°')
    end	
...
function segment.init_p_values()
    ...
    adj_elect01 = 0
  adj_elect03 = V2_voltage
  adj_elect02 = 0
;    adj_elect04 = 0
  adj_elect05 = V2_voltage
  adj_elect06 = V3_voltage
  adj_elect07 = V2_voltage
...
function segment.terminate()
  ...
  -- Acceptance criteria
	if ion_px_gu>149.9 and ion_px_gu <150.1 and ion_py_gu>-5.1 and ion_py_gu<5.1 then 
    nhits = nhits + 1
