...
-- file GEM
local gem_list = {}
for i = 0, 10, 10 do -- partenza, fine inclusa, incremento
...
local function generate_particles(k)
  local outfile = string.format("particles_%d.ion", k)
  local N = 300
  local R = 100
  local x0 = 230
  local y0 = 0
...
--=== CICLO SUI VOLTAGGI (ora cicla anche sui file .ion) ===
function segment.flym()

  -- elimina la cartella precedente se esiste
  ...
  for k=1,N_files do
    generate_particles(k)
    read_ion_file("particles_"..k..".ion")
    particles = #ion_defs
    ...
    for _, gemfile in ipairs(gem_list) do
      ...
      for i=1,V1_count do
        ...
        local fname = string.format("output/simulazione_file_%d_%s_V1_%d_V2_%d.txt",k,gemfile,V1_voltage,V2_voltage)
        ...
        --simion.printer.filename = string.format("output/results_file_%d_%s_V1_%d_V2_%d.png",k,gemfile,V1_voltage,V2_voltage)
        --simion.printer.scale = 1
        --simion.print_screen()

        --stampa risultati nel file CSV
        results_file:write("FILE="..k.."GEM="..gemfile.."V1="..V1_voltage.."  V2="..V2_voltage.."\n")
        results_file:flush()
      end
    end
  end
