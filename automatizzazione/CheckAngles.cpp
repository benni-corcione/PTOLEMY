...
int main(int argc, char* argv[]){
  ...
  if( argc!= 4 ) {
    ...
    std::cout << "EXAMPLE: ./CheckAngles cicloV root simulazione_V1_-100_V2_0" << std::endl;
    exit(1);
  }
  ...
  TFile run(Form("%s/%s/%s.root",folder,subfolder,name));
  ...
  for(int iEntry=0; iEntry<nentries; iEntry++){
    ...
    elev_hist->Fill(elev_i*M_PI/180.);
    azim_hist->Fill(azim_i*M_PI/180.);
    sen_el->Fill(sin(elev_i*M_PI/180.));
  ...
  gStyle->SetOptStat(0);
  elev_hist->SetLineWidth(3);
  azim_hist->SetLineWidth(3); 
  elev_hist->SetLineColor(kAzure+7);
  azim_hist->SetLineColor(kPink);
  
  azim_hist->GetYaxis()->SetTitle("Counts");
  azim_hist->GetXaxis()->SetTitle("Angles");
  azim_hist->SetTitle("Check of uniformity -> initial angles");
  elev_hist->Draw();
  azim_hist->Draw("same");

  legend->AddEntry(azim_hist,"althazimutale i", "l");
  legend->AddEntry(elev_hist,"elevazione i", "l");
  
  legend->Draw("same");
  
  std::string outdir( Form("%s/plots",folder));
