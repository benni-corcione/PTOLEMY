...
int main(int argc, char* argv[]){
  ...
  for(int p=0; p<pin_radius.size();  p++){
	...
	*/
	...
	gr_r->SetPoint(v,v_pin[v],d_max*1.0/pin_radius[p]);
    ...
    */
    ...
    mg_r->GetYaxis()->SetTitle("dmax / pinhole_radius");
    mg_r->GetHistogram()->SetTitle("dmax/pin_radius reached on MiC plate vs pinhole radius, cnt_radius = %d um");
    mg_r->Draw("a");
    legend_r->Draw("same");
    c2->SaveAs(Form("plots/%s/canestri/focal_dmax.png",folder));
    c2->Clear();
    legend_r->Clear();
