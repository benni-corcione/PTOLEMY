...
int main(void){ 
  ...
  int nbins2 = 200;
  int nbins1 = 100;
  tree1->SetBranchAddress("amp", &amp1);
  tree2->SetBranchAddress("amp", &amp2);

  Settings settings;
  float hmax = 1.;
  float hmin = 0.;
  float binsize_amp1 = settings.Binsize((hmax-hmin)/(1.*nbins1));
  float binsize_amp2 = settings.Binsize((hmax-hmin)/(1.*nbins2)); 

  std::cout << binsize_amp1 << std::endl;
  std::cout << binsize_amp2 << std::endl;
  //creazione istogramma e lettura del tree
  TH1F* h1= new TH1F("h1", "Amp", nbins1, hmin, hmax);
  TH1F* h2= new TH1F("h2", "Amp", nbins2, hmin, hmax);
